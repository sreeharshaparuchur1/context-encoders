...
real_center = torch.FloatTensor(1, 3, opt.imageSize, opt.imageSize)
...
random_patches=[]
for i in range(len(random_patches_no)):
    r = random_patches_no[i]//4
    c = random_patches_no[i]%4
    random_patches.append([(opt.imageSize*r)//4,(opt.imageSize*(r+1))//4,(opt.imageSize*c)//4,(opt.imageSize*(c+1))//4])

for i in random_patches:
    input_cropped.data[:,0,i[0]:i[1],i[2]:i[3]] = 2*117.0/255.0 - 1.0
    input_cropped.data[:,1,i[0]:i[1],i[2]:i[3]] = 2*104.0/255.0 - 1.0
    input_cropped.data[:,2,i[0]:i[1],i[2]:i[3]] = 2*123.0/255.0 - 1.0

fake = netG(input_cropped)
print(fake.device, real_center.device)
fake = fake.cpu()
...
utils.save_image('val_real_samples_ra.png',image[0])
utils.save_image('val_cropped_samples_ra.png',input_cropped.data[0])
utils.save_image('val_recon_samples_ra.png',fake.data[0])
...
p = p + psnr(real_center.squeeze().transpose(1,2,0) , fake.squeeze().transpose(1,2,0))
