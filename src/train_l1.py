...
for epoch in range(resume_epoch,opt.niter):
    ...
    torch.save({'epoch':epoch+1,
                ...
                'model/netlocalD_l1.pth' )
    if((epoch+1)%50==0):
        torch.save({'epoch':epoch+1,'state_dict':netG.state_dict()},'model/netG_streetview_l1'+str(epoch+1)+'.pth')
        

total_list = np.array(total_list)
adv_list = np.array(adv_list)
l2_list = np.array(l2_list)
np.save("total_loss_l1.npy", total_list)
np.save("adv_loss_l1.npy", adv_list)
np.save("l1_loss.npy", l2_list)
