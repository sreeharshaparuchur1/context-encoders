...
from model_6 import _netlocalD,_netG
import utils
...
for epoch in range(resume_epoch,opt.niter):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        random_patches_no=random.sample(range(0, 16), 5)
        random_patches=[]
        for ii in range(len(random_patches_no)):
            r = random_patches_no[ii]//4
            c = random_patches_no[ii]%4
            random_patches.append([(opt.imageSize*r)//4,(opt.imageSize*(r+1))//4,(opt.imageSize*c)//4,(opt.imageSize*(c+1))//4])
        ...
        for ii in random_patches:
            input_cropped.data[:,0,ii[0]:ii[1],ii[2]:ii[3]] = 2*117.0/255.0 - 1.0
            input_cropped.data[:,1,ii[0]:ii[1],ii[2]:ii[3]] = 2*104.0/255.0 - 1.0
            input_cropped.data[:,2,ii[0]:ii[1],ii[2]:ii[3]] = 2*123.0/255.0 - 1.0
        ...
        if i % 100 == 0:
            ...
            vutils.save_image(input_cropped.data,
                    'result/train/cropped_random/cropped_samples_epoch_%03d.png' % (epoch))
            recon_image = input_cropped.clone()
            recon_image.data = fake.data
            vutils.save_image(recon_image.data,
                    'result/train/recon_random/recon_center_samples_epoch_%03d.png' % (epoch))
    ...
    torch.save({'epoch':epoch+1,
                ...
                'model/netlocalD_random.pth' )
    if((epoch+1)%50==0):
        torch.save({'epoch':epoch+1,'state_dict':netG.state_dict()},'model/netG_streetview_random'+str(epoch+1)+'.pth')
        

total_list = np.array(total_list)
adv_list = np.array(adv_list)
l2_list = np.array(l2_list)
np.save("total_loss_random.npy", total_list)
np.save("adv_loss_random.npy", adv_list)
np.save("l2_loss_random.npy", l2_list)
